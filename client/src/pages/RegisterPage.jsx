...
const VIBE_OPTIONS = [
  'chaotic good', 'overthinking', 'main character', 'quiet menace',
  'emotionally unavailable', 'surprisingly wholesome', 'chronically online',
  'dangerous advice', 'midnight philosopher', 'accidentally iconic',
];
...
export default function RegisterPage() {
  ...
  const toggleVibe = (v) => {
    setVibeTags(prev =>
      prev.includes(v) ? prev.filter(t => t !== v) : prev.length < 5 ? [...prev, v] : prev
    );
  };
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-24 relative z-10">
        ...
        >
          <div className="text-center mb-8">
            ...
            <p className="text-muted-2">Set up your link. Let friends spill the truth.</p>
          </div>

          <form onSubmit={handleSubmit} className="glass rounded-3xl p-8 border border-border space-y-5">
            <div>
              <label className="block text-xs font-mono text-muted-2 mb-2 uppercase tracking-widest">Your Name</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-xs font-mono text-muted-2 mb-2 uppercase tracking-widest">Username</label>
              <div className="flex items-center bg-surface-2 border border-border focus-within:border-neon rounded-xl px-4 py-3 transition-colors">
                <span className="text-muted font-mono text-sm mr-1">mirror.app/u/</span>
                <input
                  type="text"
            ...
            <div>
              <label className="block text-xs font-mono text-muted-2 mb-3 uppercase tracking-widest">
                Your Vibe Tags <span className="text-muted">(pick up to 5)</span>
              </label>
